...
export default function HeroBannersTab() {
  ...
  const handleBannerSubmit = async (
    ...
    try {
      ...
      const response = await fetch(
        editingBannerId
          ? `${DOMAIN}/api/hero-banners/${editingBannerId}`
          : `${DOMAIN}/api/hero-banners`,
        {
  ...
  const handleBannerDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`${DOMAIN}/api/hero-banners/${id}`, {
        method: 'DELETE',
      });
          ...
          <div className='flex items-center gap-3'>
            <div className='flex flex-col gap-1'>
              ...
              <span className='text-[11px] text-gray-400'>
                يتحكم في لون النص داخل البانر (أبيض للصورة الداكنة، غامق للصورة
                الفاتحة)
              </span>
            </div>
            <input
