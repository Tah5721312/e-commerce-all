...
const ProductDetails = ({
  ...
  const getSizeQuantity = (sizeId: number) => {
    ...
    const variant = selectedColorData.variants.find(
      (v) => v.size?.id === sizeId
    );
              ...
              <div className='flex gap-2 overflow-x-auto pb-2 scrollbar-hide'>
                {product.productimg.map((img, index) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 md:w-20 md:h-20 p-1 border-2 rounded-lg transition-all overflow-hidden bg-gray-50 ${
                      selectedImg === index
              ...
              {availableSizes.map((size) => {
                ...
                return (
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-lg border-2 transition-all font-medium text-sm shadow-sm hover:shadow-md ${
